...
if __name__ == "__main__":
  ...
  parser.add_argument('-liveness_threshold', type=float, default=0.04, help='minimum height ratio of frame for face to occupy')
  ...
  while True:
    ...
    if ret and data is not None:
      ...
      if landmarks is not None:
        ...
        yy = thermalcrop[y_coords[3],:]   # thermal horizontal cut from nose landmark point

        if np.std(yy/np.max(yy)) > liveness_threshold:
